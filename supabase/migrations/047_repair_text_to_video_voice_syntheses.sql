-- 047_repair_text_to_video_voice_syntheses.sql
-- Repair drift: migration 044 recorded on project syijxdgekowpcboxpeyl without
-- public.text_to_video_voice_syntheses. Does not rewrite 044 history.
-- Idempotent: safe when the table already matches the production contract.
...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'text_to_video_voice_syntheses'
  ) and not exists (
    select 1
    from information_schema.columns
    where table_schema = 'public'
      and table_name = 'text_to_video_voice_syntheses'
      and column_name = 'synthesis_fingerprint'
  ) then
    raise exception
      'text_to_video_voice_syntheses exists with incompatible schema — manual review required';
  end if;
end $$;
...
drop policy if exists text_to_video_voice_syntheses_select on public.text_to_video_voice_syntheses;
create policy text_to_video_voice_syntheses_select on public.text_to_video_voice_syntheses
  for select to authenticated
  using (public.owns_project(project_id));

drop policy if exists text_to_video_voice_syntheses_insert on public.text_to_video_voice_syntheses;
create policy text_to_video_voice_syntheses_insert on public.text_to_video_voice_syntheses
  for insert to authenticated
  with check (public.owns_project(project_id));

drop policy if exists text_to_video_voice_syntheses_update on public.text_to_video_voice_syntheses;
create policy text_to_video_voice_syntheses_update on public.text_to_video_voice_syntheses
  for update to authenticated
  using (public.owns_project(project_id))
  with check (public.owns_project(project_id));

drop policy if exists text_to_video_voice_syntheses_delete on public.text_to_video_voice_syntheses;
