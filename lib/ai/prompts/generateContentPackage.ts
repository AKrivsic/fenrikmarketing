...
export function buildGenerateContentPackagePrompt(
  ...
  const visualBeatsLines = requireVideo
    ? [
        ...
        "EXCEPTION FOR PRODUCT DEMONSTRATION: when showing chat/website interaction,",
        "you MUST still depict structured UI state (sent bubble → empty waiting space →",
        "reply bubble appearing → confirmation layout) as blurred/abstract shapes so the",
        "viewer can see Question → AI answer → Result. Do NOT invent readable glyphs,",
        "and do NOT replace the interaction with a floating chat icon or a smile.",
        ...
        "  chat conversation that person started. Do not change gender, age, face, or profession.",
        ...
        "- Forbidden for variety: swapping to a different suited professional mid-package.",
        "",
        ...
        "4) Visible useful result — booking/lead/confirmation/conversation continues",
        "Resolution still MUST be step 3 or 4 — never smile-only or floating icon.",
        "",
