...
export function buildContentPackageVisualScenesBlock(args: {
  ...
  return [
    ...
    "",
    "MOTION_PROMPT (optional on IMAGE/legacy scenes; strongly preferred for video packages):",
    "- Describe visible short-form social action: what the main person/object does, ambient motion, and camera move.",
    "- State what must stay visually stable (person identity, product look, venue branding).",
    "- Do NOT add character speech or lip-sync (voiceover is separate).",
    "- Do NOT invent on-screen text, titles, logos, or body/product deformations unless already in the source still.",
    "- Keep motion_prompt under 1000 characters (UTF-16 / JS string length).",
    "- Optional transition_in on a scene: one of fade | slide | push | none (incoming clip transition).",
    "",
    ...
    visual_scenes: [
      {
        source: "ai",
        image_prompt: "string",
        motion_prompt: "optional motion string ≤1000",
        transition_in: "optional fade|slide|push|none",
      },
      {
        source: "asset",
        asset_id: "uuid",
        used_as: "string",
        motion_prompt: "optional",
      },
    ],
