...
export function buildProductDemonstrationPromptBlock(
  ...
  return [
    ...
    "  conversation_id: stable id for this demonstration thread",
    "  question_visible: true   // input / initial state visible",
    "  ai_answer_visible: true  // product/service value creation visible",
    ...
    "  (Field names reflect the current chat execution; semantics are input → value → outcome.)",
    "",
    "Include one visual_scenes entry:",
    '  { "type": "PRODUCT_DEMO", "payload": { ...product_demo fields } }',
    "Place PRODUCT_DEMO after the opening meaning block — not as visual_scenes[0]",
    "and not as a sales/CTA opening. Product may appear in the hook situation;",
    "sales pitch / offer / pricing as the first spoken meaning unit is forbidden.",
    "Current production renderer executes conversational products as a controlled chat UI.",
    "Do NOT invent empty bubbles, floating chat icons, smile-only results, logo-only,",
    "or landing-page-only demos. Do not fabricate a chatbot demonstration for products",
    "that do not operate as conversational interfaces.",
    "",
...
export function productDemonstrationRepairAppendix(
  ...
  return [
    ...
    "",
    "Replace the failing resolution with ONE complete PRODUCT_DEMO scene that satisfies:",
    "  input → product/service creates value → visible outcome",
    "  (structured product_demo beat — do not rely on prose alone).",
    "Do not invent a chatbot demonstration for unrelated products.",
    "Do not use smile / floating icon / landing page as the demo.",
  ].join("\n");
}
