...
export async function preparePhoneSceneRaster(
  ...
  if (scene.image_bucket && scene.image_path) {
    ...
    try {
      ...
      if (ctx.forbidImageGeneration || ctx.isLanguageVariant) {
        throw new Error(
          `language_variant_visual_asset_missing: scene ${scene.id} failed to download phone raster (${message})`,
        );
      }
      throw err;
    }
  ...
  if (ctx.forbidImageGeneration || ctx.isLanguageVariant) {
    throw new Error(
      `language_variant_visual_asset_missing: scene ${scene.id} missing durable phone raster; image generation forbidden for language variants`,
    );
  }
