...
export async function prepareChecklistSceneRaster(
  ...
  if (scene.image_bucket && scene.image_path) {
    ...
    try {
      ...
      if (ctx.forbidImageGeneration || ctx.isLanguageVariant) {
        throw new Error(
          `language_variant_visual_asset_missing: scene ${scene.id} failed to download checklist raster (${message})`,
        );
      }
      throw err;
    }
  ...
  if (ctx.forbidImageGeneration || ctx.isLanguageVariant) {
    throw new Error(
      `language_variant_visual_asset_missing: scene ${scene.id} missing durable checklist raster; recomposition forbidden for language variants`,
    );
  }
